...
export const VAT_RATES = {
  ...
  SECOND_REDUCED_9: {
    ...
    description:
      "Newspapers, periodicals, e-books, admission to cultural/sports events, gas & electric (until 30/04/2025)",
  },
  LIVESTOCK_4_8: {
    key: "livestock_4_8",
    rate: 0.048,
...
export const DISALLOWED_VAT_CREDITS = {
  ...
  PASSENGER_VEHICLES: {
    ...
    vatRecoverable: false,
    keywords: ["car purchase", "car lease", "car hire", "car rental", "motor finance", "pcp", "hp car"],
  },
 ...
 */
export function applyTwoThirdsRule(
  partsVatExclusiveCost: number,
  totalChargeVatExclusive: number,
): {
...
export const INDUSTRY_VAT_RULES: Record<
  string,
  {
...
> = {
  ...
  retail_ecommerce: {
    ...
    specialRules: ["Most retail goods at 23%", "Children's clothing/shoes at 0%", "Books at 0%", "Most food at 0%"],
  },
  professional_services: {
    defaultOutputRate: "standard_23",
    commonInputRates: ["standard_23", "exempt"],
    specialRules: ["Professional services at 23%", "Some financial services exempt"],
  },
  technology_it: {
    defaultOutputRate: "standard_23",
    commonInputRates: ["standard_23", "exempt"],
...
export function determineVatTreatment(
  ...
  if (transactionType === "expense") {
    ...
    if (
      DISALLOWED_VAT_CREDITS.PETROL.keywords.some((k) => descLower.includes(k)) &&
    ...
    if (fuelStations.some((f) => descLower.includes(f))) {
      return {
        ...
        needsReceipt: true,
        explanation:
          "Fuel station - need receipt to determine if diesel (VAT recoverable) or petrol/food (not recoverable)",
