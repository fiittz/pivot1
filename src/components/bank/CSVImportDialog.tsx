...
const CSVImportDialog = ({ onImportComplete, selectedFinancialAccountId }: CSVImportDialogProps) => {
  ...
  const parseDate = (dateStr: string): string => {
    const formats = ["dd/MM/yyyy", "MM/dd/yyyy", "yyyy-MM-dd", "dd-MM-yyyy", "d/M/yyyy", "dd MMM yyyy"];
  ...
  const parseTransactionsFromMapping = (data: string[][], hdrs: string[], map: ColumnMapping): ParsedTransaction[] => {
    ...
    if (!map.date || !map.description || (!hasAmount && !hasCredit)) {
      console.log("[CSV Import] Missing required mapping:", {
        date: map.date,
    ...
    const results = nonEmptyRows
      .map((row, idx) => {
        ...
        if (row.length <= neededIdx) {
          droppedShortRow++;
          console.log(
            `[CSV Import] Row ${idx + 1} too short: ${row.length} cols, need ${neededIdx + 1}. Raw: ${row.join("|").substring(0, 100)}`,
          );
          return null;
        }
    ...
    console.log(
      `[CSV Import] Parse summary: ${nonEmptyRows.length} rows → ${results.length} transactions (${droppedShortRow} short rows, ${droppedZeroAmount} zero-amount)`,
    );
    return results;
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const delimiter =
          tabCount >= commaCount && tabCount >= semicolonCount ? "\t" : semicolonCount > commaCount ? ";" : ",";

        console.log(
          `[CSV Import] Lines: ${lines.length}, Delimiter: "${delimiter === "\t" ? "TAB" : delimiter}", Tabs: ${tabCount}, Commas: ${commaCount}, Semicolons: ${semicolonCount}`,
        );
        ...
        console.log(
          `[CSV Import] First row col count: ${parsed[1]?.length}, Last row col count: ${parsed[parsed.length - 1]?.length}`,
        );
        console.log(
          `[CSV Import] Total data rows: ${parsed.length - 1}, Valid rows (>=2 cells): ${validData.length}, Dropped: ${parsed.length - 1 - validData.length}`,
        );
        ...
        if (validData.length > 0) {
          console.log(
            "[CSV Import] First data row sample:",
            validData[0].map((v, i) => `[${i}]${cleanHeaders[i]}=${v}`).join(", "),
          );
        }
        ...
        const isRevolut = headersLower.some(
          (h) => (h.includes("completed") && h.includes("date")) || (h.includes("started") && h.includes("date")),
        );
        ...
        if (isAIB) {
          ...
          const descCol =
            cleanHeaders.find(
              (_, i) => headersLower[i] === "description1" || headersLower[i].includes("description"),
            ) || "";
          ...
          const dateCol =
            cleanHeaders.find((_, i) => headersLower[i].includes("completed") && headersLower[i].includes("date")) ||
          ...
          const dateCol =
            cleanHeaders.find((_, i) => headersLower[i].includes("date") && !headersLower[i].includes("balance")) || "";
          const descCol =
            cleanHeaders.find(
              (_, i) =>
                headersLower[i].includes("description") ||
                headersLower[i].includes("narrative") ||
                headersLower[i].includes("details"),
            ) || "";
          const amountCol = cleanHeaders.find((_, i) => headersLower[i] === "amount") || "";
          const creditCol =
            cleanHeaders.find((_, i) => headersLower[i].includes("credit") && !headersLower[i].includes("card")) || "";
          const debitCol = cleanHeaders.find((_, i) => headersLower[i].includes("debit")) || "";
        ...
        if (autoMapping) {
          ...
          if (transactions.length > 0) {
            ...
            toast.success(
              `${bankName ? bankName + " format detected! " : ""}${transactions.length} of ${validData.length} transactions ready${dropMsg}`,
            );
            setStep("preview");
          } else {
    ...
    try {
      ...
      const existingFingerprints = new Set(
        existingTransactions?.map(
          (t) => `${t.transaction_date}|${t.description.toLowerCase().trim()}|${Math.abs(t.amount).toFixed(2)}`,
        ) || [],
      );
  ...
  const handleImport = async () => {
    ...
    toast.success(
      `Imported ${success} of ${transactionsToImport.length} transactions${skippedMsg}${failMsg}. Now auto-categorizing...`,
    );
    if (failedCount > 0) {
    ...
    if (createdTransactions.length > 0) {
      ...
      if (categoriesToUse.length === 0) {
        ...
        const { data: fetchedCategories } = await supabase.from("categories").select("*").order("name");
    ...
    if (trips.length > 0) {
      ...
      const matched = invoiceTrips.filter((it) => trips.some((t) => it.matchedTrip?.id === t.id));
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select
                    value={mapping.credit || "__none__"}
                    onValueChange={(v) => setMapping((m) => ({ ...m, credit: v === "__none__" ? "" : v }))}
                  >
                    <SelectTrigger className="mt-1">
                    ...
                    <SelectContent>
                      ...
                      {headers.map((h, i) => (
                        <SelectItem key={`credit-${i}`} value={h}>
                          {h}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div>
                  ...
                  <Select
                    value={mapping.debit || "__none__"}
                    onValueChange={(v) => setMapping((m) => ({ ...m, debit: v === "__none__" ? "" : v }))}
                  >
                    <SelectTrigger className="mt-1">
                      <SelectValue placeholder="Optional" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__none__">None</SelectItem>
                      {headers.map((h, i) => (
                        <SelectItem key={`debit-${i}`} value={h}>
                          {h}
                        </SelectItem>
                      ))}
                    </SelectContent>
              ...
              <div>
                ...
                <Select
                  value={mapping.reference || "__none__"}
                  onValueChange={(v) => setMapping((m) => ({ ...m, reference: v === "__none__" ? "" : v }))}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder="Optional" />
                  </SelectTrigger>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`font-medium truncate ${willBeSkipped ? "line-through" : ""}`}>{t.description}</p>
        ...
        {step === "importing" && (
          <div className="py-8 text-center space-y-4">
            ...
            {isCategorizing && (
              <p className="text-xs text-muted-foreground">AI is analyzing merchants and assigning categories...</p>
            )}
